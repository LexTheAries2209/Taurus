...
struct DITModeComparisonView: View {
    ...
    private func comparisonColumn(_ item: PlanItem) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            ComparisonValue(label: "每张卡时长", value: summary.map { "\(formatNumber($0.recordMinutesPerMedia)) 分钟" } ?? "—")
            ComparisonValue(label: "卸载时间", value: summary.map { "\(formatNumber($0.transferSeconds / 3_600)) 小时" } ?? "—")
        }
        .padding(12)
