...
enum DITPlanExport {
    ...
    static func csvData(for project: DITProject) -> Data {
        ...
        let summaryByID = Dictionary(uniqueKeysWithValues: summary.itemSummaries.map { ($0.itemID, $0) })
        var rows: [[String]] = [[
    ...
    private static func reportAttributedText(project: DITProject, summary: PlanSummary) -> NSAttributedString {
        ...
        let titleAttributes: [NSAttributedString.Key: Any] = [
            .font: NSFont(name: "PingFangSC-Semibold", size: 22) ?? NSFont.boldSystemFont(ofSize: 22),
            .foregroundColor: NSColor.black
        ]
        ...
        let sectionAttributes: [NSAttributedString.Key: Any] = [
            .font: NSFont(name: "PingFangSC-Semibold", size: 14) ?? NSFont.boldSystemFont(ofSize: 14),
            .foregroundColor: NSColor.black,
            .paragraphStyle: sectionStyle
        ]
        let itemAttributes: [NSAttributedString.Key: Any] = [
            .font: NSFont(name: "PingFangSC-Semibold", size: 11) ?? NSFont.boldSystemFont(ofSize: 11),
            .foregroundColor: NSColor.black,
            .paragraphStyle: bodyStyle
        ]
        ...
        appendLine("读卡器：\(number(project.transferProfile.readerSpeedMBps)) MB/s", attributes: bodyAttributes, to: output)
        appendLine("目标盘：\(number(project.transferProfile.targetDiskSpeedMBps)) MB/s", attributes: bodyAttributes, to: output)
        appendLine("每日卸载窗口：\(number(project.transferProfile.offloadWindowHoursPerDay)) 小时", attributes: bodyAttributes, to: output)
        appendLine("有效速度：\(number(summary.effectiveTransferSpeedMBps)) MB/s", attributes: bodyAttributes, to: output)

        appendLine("机位明细", attributes: sectionAttributes, to: output)

        for itemSummary in summary.itemSummaries {
            guard let item = project.items.first(where: { $0.id == itemSummary.itemID }) else { continue }
    ...
    private static func drawFooter(pageNumber: Int, in context: CGContext, pageRect: CGRect) {
        ...
        let path = CGPath(rect: CGRect(x: 44, y: 24, width: pageRect.width - 88, height: 14), transform: nil)
