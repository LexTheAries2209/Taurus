...
func Comments(cameradata: CameraData, language: AppLanguage) -> some View {
    let copy = language.copy
    let commentLines = copy.comments(for: cameradata.BrandName, cameraName: cameradata.CameraName)
    let bodyFontSize: CGFloat = language == .english ? 11 : 10
    let lineSpacing: CGFloat = language == .english ? 2 : 1
